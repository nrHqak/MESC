...
### Быстро через интерфейс (без ручного JSON)
- Откройте `Загрузка файлов`.
- Выберите файл (`pdf/docx/txt/pptx`).
- Укажите предмет и название материала.
- Приложение автоматически добавит материал в `Data/materials.json` для выбранного класса, и он появится во вкладке `Материалы` нужного предмета.
